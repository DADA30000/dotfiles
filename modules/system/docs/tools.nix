...
with lib;
let
  nix-man = (pkgs.runCommand "fixup manual" { manual = config.system.build.manual.nixos-configuration-reference-manpage; } ''
    mkdir -p $out
  ...
  stable-manual = (inputs.nixpkgs-stable.lib.nixosSystem { modules = [ { nixpkgs.hostPlatform = lib.mkDefault "x86_64-linux"; } ]; }).config.system.build.manual.manualHTML;
  nos_unwrapped = (pkgs.callPackage "${inputs.nos}/default.nix" { inherit pkgs; gitignoreSrc = { gitignoreSource = (x: x); }; }).overrideAttrs { patches = [ ../../../stuff/nos.patch ]; };
...
in
{
  config = mkIf cfg.enable {
    ...
    environment.systemPackages = mkIf cfg.nos.enable [ nos man-nix man-home ];
  };
}
