...
with lib;
let
  standardBuildOptionsDocs = args@{ modules, ... }:
    let
      poisonModule = { options, ... }: {
        config = listToAttrs (map (n: { name = n; value = abort "documentation depends on config"; })
          (filter (n: n != "_module") (attrNames options)));
      };
      evaled = lib.evalModules {
        modules = modules ++ [ poisonModule { _module.check = false; } ];
        class = "homeManager";
  ...
  lib-hm = (import "${inputs.home-manager}/modules/lib/stdlib-extended.nix" lib).extend (_: _: { mkDoc = s: s; });
  ...
  customBuildOptionsDocs = args@{ modules, ... }:
    let
      evaled = lib-hm.evalModules {
        modules = modules ++ [ { config = { home.stateVersion = config.home-manager.users.${user}.home.stateVersion; }; } ];
        class = "homeManager";
  ...
  hm-manpage = (pkgs.runCommand "hm-custom-manpage" { nativeBuildInputs = [ pkgs.nixos-render-docs ]; inherit revision; } ''
  ...
  hm-html = (pkgs.callPackage "${inputs.home-manager}/docs/home-manager-manual.nix" {
    inherit revision;
  ...
  hm-html-opener = pkgs.callPackage "${inputs.home-manager}/docs/html-open-tool.nix" {} { html = hm-html; };
  cfg = config.docs;
in
{
  config = mkIf cfg.enable {
    ...
    environment.systemPackages = [ hm-manpage hm-html-opener hm-html ];
